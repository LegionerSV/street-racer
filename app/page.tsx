...
import { ArrowUpRight, CornerUpLeft, CornerUpRight, Flag, MapPin, Route, Pause, Volume2 } from 'lucide-react';
...
export default function Home() {
  ...
  {stage === 'playing' && hud && world && <div className={`game-hud ${touchVisible?'touch-layout':''} ${hud.boosting?'nitro-active':''}`}>
    ...
    {hud.nearRace && !hud.race && <button className="race-invite" onClick={() => gameRef.current?.startRace(hud.nearRace!)}><span className="keycap">E</span><div><small>{hud.nearRace.kind === 'circuit' ? 'КОЛЬЦЕВОЙ ЗАЕЗД · 3 КРУГА' : 'СПРИНТ · 3 СОПЕРНИКА'}</small><b>{hud.nearRace.title}</b><span>Маршрут по загруженной карте · начать заезд</span></div><Flag size={25} /></button>}
    ...
    {touchVisible&&!hud.paused&&hud.race?.phase!=='countdown'&&hud.race?.phase!=='finished'&&<TouchControls onInput={touchInput} onRecover={()=>gameRef.current?.recover()} nitro={hud.nitro??1}/>}
  ...
  <Dialog open={stage === 'playing' && !!hud?.paused && !licenses} onOpenChange={open => { if (!open && gameRef.current?.paused) gameRef.current.togglePause(); }}><DialogContent className="pause-menu" showCloseButton={false}><DialogTitle>Небольшая остановка.</DialogTitle><DialogDescription>Район ждёт. Продолжи поездку или настрой игру.</DialogDescription>
    ...
    <fieldset><legend>Подсказки навигатора в гонке</legend><RadioGroup value={settings.navigator===false?'off':'on'} onValueChange={v=>changeSettings({...settings,navigator:v==='on'})} className="quality-options">{[['on','Включены'],['off','Выключены']].map(([value,label])=><label key={value}><RadioGroupItem value={value}/>{label}</label>)}</RadioGroup></fieldset>
