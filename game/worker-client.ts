...
export class WorldWorker {
  ...
  constructor() {
    this.worker.onmessage = (event: MessageEvent<WorkerResponse>) => {
      ...
      if (response.type === 'error') task.reject(new Error(response.error)); else task.resolve(response.type === 'world' ? response.world : response.chunk);
    };
  ...
  private request<T>(request: Omit<Extract<WorkerRequest, { type: 'world' }>, 'id'> | Omit<Extract<WorkerRequest, { type: 'chunk' }>, 'id'>): Promise<T> {
