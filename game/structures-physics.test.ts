...
it('машина проезжает по набережной под поднятым мостом без столкновения с плитой или опорами', async () => {
  ...
  const world = buildWorld({
    ...
    elements: [
      ...
      { type: 'way', id: 12, nodes: [3, 4], tags: { highway: 'primary' } },
      {
        ...
        id: 13,
        nodes: [5, 6],
        tags: { highway: 'secondary', width: '14' },
      },
    ],
  });
  ...
  car.teleport({ x: -126, y: 1, z: -90 }, 0);
  try {
    // Act
    for (let i = 0; i < 1200 && car.position.z < 90; i++) {
      car.step(1 / 60, new Set(car.speed < 14 ? ['KeyW'] : []), false);
      scene.getPhysicsEngine()!._step(1 / 60);
      car.afterPhysics();
    }
    // Assert
    expect(car.position.z).toBeGreaterThan(85);
...
it.each([
  ...
  'машина проходит %s целиком с подходами (портал %s°)',
  async (kind, portalLon) => {
    // Arrange
    const world = buildWorld({
      center: { lat: 0, lon: 0 },
      ...
      elevation: { size: 5600, width: 2, values: new Float32Array(4) },
      elements: [
        { type: 'node', id: 1, lat: 0, lon: -0.006 },
        { type: 'node', id: 2, lat: 0, lon: -portalLon },
        { type: 'node', id: 3, lat: 0, lon: portalLon },
        { type: 'node', id: 4, lat: 0, lon: 0.006 },
        {
          type: 'way',
          id: 10,
          nodes: [1, 2],
    ...
    const car = new PlayerCar(scene);
    car.teleport({ x: -endX, y: 1, z: -1 }, Math.PI / 2);
    let lowest = Infinity,
      highest = -Infinity;
    try {
      // Act
      for (let i = 0; i < 4200 && car.position.x < endX; i++) {
        ...
        car.step(1 / 60, keys, false);
        scene.getPhysicsEngine()!._step(1 / 60);
        car.afterPhysics();
        if (Math.abs(car.position.x) < portalLon * 111320 * 0.35) {
          lowest = Math.min(lowest, car.position.y);
          highest = Math.max(highest, car.position.y);
        }
      }
      // Assert
      expect(car.position.x).toBeGreaterThan(endX - 5);
      expect(car.grounded).toBe(true);
