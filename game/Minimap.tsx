...
export function Minimap({ world, hud, mobile=false }: { world: World; hud: HUD; mobile?:boolean }) {
  ...
  useEffect(() => {
    ...
    if (hud.race) {
      ...
      const p = world.edges[world.spawnEdge].points[0], [x, y] = transform(p.x, p.z); ctx.fillStyle = '#d8ff3e'; ctx.beginPath(); ctx.arc(x, y, 5, 0, 2 * Math.PI); ctx.fill();
    }
  ...
  return <canvas className="minimap" ref={ref} width={300} height={240} aria-label="Мини-карта района и маршрут заезда" />;
}
