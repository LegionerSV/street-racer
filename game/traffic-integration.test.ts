...
it('соперники получают спортивный разгон и трафик становится подвижным до столкновения',async()=>{
  ...
  const route:Route={id:'acceleration',kind:'sprint',title:'Разгон',edges:['edge-0'],points:w.edges[0].points,cumulative:[0,400],length:400,laps:1};
  ...
  try{
    ...
    traffic.clearRacers();traffic.agents.push({id:12,edge:'edge-0',distance:25,speed:0,point:{x:-1.7,y:.96,z:25},heading:0,stuck:0});
...
it.each([
  ...
  // Arrange
  const { engine, scene } = await setup(), w = world([road(0, 1, 2, 0, 400)]), traffic = new Traffic(scene, w);
  const route: Route = { id: 'avoidance', kind: 'sprint', title: 'Объезд', edges: ['edge-0'], points: w.edges[0].points, cumulative: [0, 400], length: 400, laps: 1 };
  traffic.agents.push({
    id: 12,
    edge: 'edge-0',
    distance: 20,
...
it.each([
  ...
  const route: Route = { id: 'shoulder', kind: 'sprint', title: 'Обочина', edges: ['edge-0'], points: narrow.points, cumulative: [0, 400], length: 400, laps: 1 };
  traffic.agents.push({ id: 12, edge: 'edge-0', distance: 20, speed: 12, point: { x: 0, y: .96, z: 20 }, heading: 0, stuck: 0, ...(race ? { race: { route, index: 0, lap: 1, finished: false, progress: 0 } } : {}) });
  try {
    // Act
