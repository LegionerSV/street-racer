...
export function facadeSurface(style: 'brick' | 'stone' | 'modern', size: number) {
  ...
  for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
    ...
    const stoneTone = seeded(Math.floor(x / 48) + Math.floor(y / 8) * 29) - .5;
    const shade = style === 'brick'
...
export function asphaltSurface(size: number) {
  const colour = new Uint8Array(size * size * 4), specular = new Uint8Array(colour.length);
  const height = new Float32Array(size * size);
  const patches = Float32Array.from({ length: 64 }, (_, index) => seeded(index * 71));
  const patchAt = (x: number, y: number) => patches[((y + 8) % 8) * 8 + (x + 8) % 8];
  for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
    const index = y * size + x, offset = index * 4;
    const grain = seeded(index * 19), gx = x * 8 / size, gy = y * 8 / size;
    const ix = Math.floor(gx), iy = Math.floor(gy), tx = gx - ix, ty = gy - iy;
    const smoothX = tx * tx * (3 - 2 * tx), smoothY = ty * ty * (3 - 2 * ty);
    const patch = (patchAt(ix, iy) * (1 - smoothX) + patchAt(ix + 1, iy) * smoothX) * (1 - smoothY)
      + (patchAt(ix, iy + 1) * (1 - smoothX) + patchAt(ix + 1, iy + 1) * smoothX) * smoothY;
    const shade = Math.round(170 + grain * 55 + patch * 30);
    colour[offset] = colour[offset + 1] = colour[offset + 2] = shade;
    colour[offset + 3] = 255;
    height[index] = grain * .6 + patch * .3;
    const sheen = Math.round(55 + (1 - grain) * 32 + patch * 75);
    specular[offset] = specular[offset + 1] = specular[offset + 2] = sheen;
    specular[offset + 3] = 255;
  }
  return { colour, normal: normalMap(height, size, .45), specular };
}
