...
export function trafficBudget(
  ...
  return Math.min(
    ...
    Math.max(
      0,
      Math.floor(
        meters / (density === 'rush' ? 22 : density === 'city' ? 35 : 70),
      ),
    ),
  );
}
export class Traffic {
  ...
  private refreshSignalApproaches() {
    ...
    for (const node of this.world.nodes)
      for (const approach of signalApproaches(this.world, node.id))
        this.signalAxes.set(approach.edge, approach.axis);
  }
  replaceWorld(world: World) {
    if (this.agents.some((a) => a.race))
      throw new Error('Нельзя менять дорожную сеть во время гонки.');
    this.agents = this.agents.filter((a) => {
      if (!edgeById(world, a.edge) || edgeById(world, a.edge)!.blocked) {
        this.hide(a);
        return false;
      }
      a.plan = undefined;
  ...
  applyWorldPatch(world: World, patch: WorldPatch) {
    const racers = this.agents.filter((a) => a.race);
    if (
      racers.some((a) =>
        a.race!.route.edges.some(
          (id) => !edgeById(world, id) || edgeById(world, id)!.blocked,
        ),
      )
    )
      throw new Error('Потоковое обновление потеряло участок активной трассы.');
    const affected = new Set<EdgeStableId>([
        ...patch.edgesRemoved,
        ...patch.edgesAddedOrUpdated.map(edgeStableId),
      ]),
      restrictionWays = new Set(
        [
          ...patch.restrictionsAddedOrUpdated,
          ...patch.restrictionsRemoved,
        ].flatMap((r) => [r.fromWay, r.toWay, ...(r.viaWays ?? [])]),
      ),
      restrictionsChanged = restrictionWays.size > 0;
    this.agents = this.agents.filter((a) => {
      if (a.race) return true;
      const edge = edgeById(world, a.edge);
      ...
      const planAffected =
        ...
        a.plan?.ids.some(
          (id) =>
            affected.has(id) ||
            restrictionWays.has(edgeById(world, id)?.way ?? -1),
        );
      if (planAffected) {
        a.plan = undefined;
        a.travel = a.distance;
    ...
    this.refreshSignalApproaches();
    if (
      patch.nodesAddedOrUpdated.some((n) => n.signal) ||
      patch.nodesRemoved.length ||
      restrictionsChanged
    )
      this.reservations.clear();
  }
  setDensity(d: 'light' | 'city' | 'rush') {
    this.density = d;
    this.spawnTimer = 0;
  }
  private makePlan(agent: Agent) {
    const ids = agent.race
      ? Array.from(
          ...
          () => agent.race!.route.edges,
        ).flat()
      : [agent.edge];
    if (agent.plan && !agent.race) ids.splice(0, ids.length, ...agent.plan.ids);
    let total = ids.reduce(
      (sum, id) => sum + edgeById(this.world, id)!.length,
      0,
    );
    let history: number[] = [];
    for (const id of ids)
      history = advanceTurnHistory(
        this.world,
        history,
        edgeById(this.world, id)!.way,
      );
    const goal = agent.race ? total + 100 : (agent.travel || 0) + 700;
    let deadEnd = false;
    for (let n = 0; total < goal && n < 600; n++) {
      const edge = edgeById(this.world, ids.at(-1)!)!;
      let options = outgoing(this.world, edge.to).filter(
        (e) => e.to !== edge.from && allowedTurn(this.world, edge, e, history),
      );
      if (!options.length) {
        deadEnd = true;
        break;
      ...
      const next =
        options[
          Math.floor(
            seeded(agent.id * 91 + edge.to + ids.length) * options.length,
          )
        ];
      ids.push(edgeStableId(next));
      total += next.length;
      history = advanceTurnHistory(this.world, history, next.way);
    }
    const points: Point[] = [],
      ends: number[] = [];
    let length = 0;
    for (const id of ids) {
      const edge = edgeById(this.world, id)!;
      points.push(...(points.length ? edge.points.slice(1) : edge.points));
      length += edge.length;
      ends.push(length);
    }
    agent.plan = {
      ...
      path: smoothPath(
        points,
        Math.min(10, edgeById(this.world, agent.edge)!.width * 0.75),
      ),
      exhausted: deadEnd,
    };
  ...
  private preferredLane(agent: Agent, edge: Edge) {
    return agent.race && agent.plan
      ? racingLineOffset(
          agent.plan.path,
          agent.travel || 0,
          edge,
          agent.race.traits || DEFAULT_RACER_TRAITS,
          agent.id,
        )
      : this.homeLane(agent, edge);
  }
  private lane(agent: Agent, edge: Edge) {
    return agent.avoidanceOffset !== undefined &&
      agent.avoidanceWay === edge.way &&
      !edge.bridge &&
      !edge.tunnel
      ? agent.avoidanceOffset
      : this.preferredLane(agent, edge);
  }
  private avoidanceLanes(agent: Agent, edge: Edge) {
    const current = this.lane(agent, edge),
      legal = laneOffsets(edge, this.world.drivingSide)
        .filter((offset) => Math.abs(offset - current) > 0.2)
        .sort((a, b) => Math.abs(a - current) - Math.abs(b - current));
    if (!agent.race || edge.bridge || edge.tunnel) return legal;
    const shoulder =
      edge.width / 2 +
      0.35 +
      (agent.race?.traits?.aggression ?? DEFAULT_RACER_TRAITS.aggression) *
        1.15;
    const shoulders = [shoulder, -shoulder].sort(
      (a, b) => Math.abs(a - current) - Math.abs(b - current),
    );
    return [...legal, ...shoulders].filter(
      (offset, index, all) =>
        all.findIndex((other) => Math.abs(other - offset) < 0.2) === index,
    );
  }
  private sample(agent: Agent, dt = 1) {
    if (!agent.plan) this.makePlan(agent);
    const plan = agent.plan!,
      d = clamp(agent.travel || 0, 0, plan.total);
    let idx = plan.ends.findIndex((end) => end > d);
    if (idx < 0) idx = plan.ids.length - 1;
    agent.edge = plan.ids[idx];
    agent.distance = d - (idx ? plan.ends[idx - 1] : 0);
    const s = samplePath(plan.path, d, true),
      ahead = samplePath(plan.path, Math.min(plan.total, d + 12), true);
    const delta = Math.atan2(
      Math.sin(ahead.heading - s.heading),
      Math.cos(ahead.heading - s.heading),
    );
    agent.turn = Math.abs(delta) > 0.18 ? Math.sign(delta) : 0;
    const offset = this.lane(agent, edgeById(this.world, agent.edge)!);
    agent.laneOffset ??= offset;
    agent.laneOffset += clamp(offset - agent.laneOffset, -dt * 1.5, dt * 1.5);
    agent.point = {
      x: s.point.x + Math.cos(s.heading) * agent.laneOffset,
      y: s.point.y + 0.84,
    ...
    if (agent.race) {
      ...
      r.index = idx % r.route.edges.length;
      r.lap = Math.min(r.route.laps, Math.floor(d / graphLength) + 1);
      r.progress = Math.min(
        (d / graphLength) * r.route.length,
        r.route.length * r.route.laps,
      );
    }
  }
  ...
  clearRacers() {
    for (const a of this.agents.filter((a) => a.race)) this.hide(a);
    this.agents = this.agents.filter((a) => !a.race);
  }
  get racers() {
  ...
  update(
    ...
    if (this.spawnTimer <= 0) {
      ...
      const keep = new Set(
        this.agents
          .filter((a) => !a.race)
          .sort(
            (a, b) => distance2(a.point, player) - distance2(b.point, player),
          )
          .slice(0, trafficBudget(Infinity, this.density, this.mobile)),
      );
      this.agents = this.agents.filter((a) => {
        if (a.race || keep.has(a)) return true;
        this.hide(a);
        return false;
      });
    ...
    for (const a of this.agents) {
      ...
      if (
        !a.race &&
        !a.plan!.exhausted &&
        a.plan!.total - (a.travel || 0) < 120
      ) {
        const cut = a.plan!.ends.findIndex((end) => end > (a.travel || 0) - 80);
        if (cut > 0) {
          const removed = a.plan!.ends[cut - 1];
          a.plan!.ids.splice(0, cut);
          a.travel = (a.travel || 0) - removed;
        }
        this.makePlan(a);
      }
      const plan = a.plan!,
        edge = edgeById(this.world, a.edge)!,
        traits = a.race?.traits || DEFAULT_RACER_TRAITS;
      if (
        a.avoidanceWay !== undefined &&
        (a.avoidanceWay !== edge.way || edge.bridge || edge.tunnel)
      ) {
        a.avoidanceOffset = undefined;
        a.avoidanceWay = undefined;
      }
      let gap = Infinity,
        leadSpeed = Infinity,
        conflictStop = Infinity,
        blockedByPlayer = false;
      const scanDistance = a.race ? 28 + traits.reaction * 42 : 60;
      for (const b of neighbors)
        if (b.agent !== a && Math.abs(b.point.y - a.point.y) < 2.5) {
          const dx = b.point.x - a.point.x,
            dz = b.point.z - a.point.z,
            along = dx * Math.sin(a.heading) + dz * Math.cos(a.heading),
            lateral = Math.abs(
              dx * Math.cos(a.heading) - dz * Math.sin(a.heading),
            );
          const aligned = Math.cos(b.heading - a.heading) > 0.45;
          if (
            aligned &&
            along > 0 &&
            along < scanDistance &&
          ...
          if (!a.race && !aligned) {
            const conflict = trajectoryConflict(
              { point: a.point, heading: a.heading, speed: a.speed },
              { point: b.point, heading: b.heading, speed: b.speed },
            );
            if (conflict)
              conflictStop = Math.min(
                conflictStop,
                Math.max(0, conflict.distance - 6),
              );
          }
        }
      const dx = player.x - a.point.x,
        dz = player.z - a.point.z,
        along = dx * Math.sin(a.heading) + dz * Math.cos(a.heading),
        lateral = Math.abs(dx * Math.cos(a.heading) - dz * Math.sin(a.heading));
      if (
        along > 0 &&
        lateral < 3 &&
        Math.abs(player.y - a.point.y) < 2.5 &&
        along < gap
      ) {
        gap = along;
      ...
      if (!a.race && Math.abs(player.y - a.point.y) < 2.5) {
        const conflict = trajectoryConflict(
          { point: a.point, heading: a.heading, speed: a.speed },
          {
            point: player,
            heading: playerHeading,
            speed: Math.abs(playerSpeed),
          },
        );
        if (conflict) {
          conflictStop = Math.min(
            conflictStop,
            Math.max(0, conflict.distance - 7),
          );
          blockedByPlayer = true;
        }
      }
      const laneClear = (lane: number) => {
        const shift = lane - (a.laneOffset ?? this.lane(a, edge)),
          margin = a.race ? 2.35 - traits.aggression * 0.75 : 2.5,
          rear = a.race ? -(16 - traits.aggression * 10) : -14,
          front = a.race ? 45 - traits.aggression * 12 : 45;
        const clear = neighbors.every((b) => {
          if (b.agent === a || Math.abs(b.point.y - a.point.y) > 3) return true;
          const dx = b.point.x - a.point.x,
            dz = b.point.z - a.point.z;
          const ahead = dx * Math.sin(a.heading) + dz * Math.cos(a.heading),
            lateral =
              dx * Math.cos(a.heading) - dz * Math.sin(a.heading) - shift;
          return ahead < rear || ahead > front || Math.abs(lateral) > margin;
        });
        const playerLateral =
          dx * Math.cos(a.heading) - dz * Math.sin(a.heading) - shift;
        const playerClear =
          Math.abs(player.y - a.point.y) > 3 ||
          along < rear ||
          along > front ||
      ...
      for (
        ...
        i < plan.ids.length && plan.ends[i] - (a.travel || 0) < 65;
        i++
      ) {
        const approaching = edgeById(this.world, plan.ids[i])!,
          points = approaching.points,
          b = points.at(-1)!,
          c = points.at(-2)!;
        const axis =
            this.signalAxes.get(plan.ids[i]) ??
            (Math.abs(b.x - c.x) > Math.abs(b.z - c.z) ? 1 : 0),
          dist = plan.ends[i] - (a.travel || 0);
        const nextEdge = plan.ids[i + 1]
          ? edgeById(this.world, plan.ids[i + 1])
          : undefined;
        if (
          nextEdge &&
          (nextEdge.laneProfile?.shared ??
            (!nextEdge.oneWay && nextEdge.width < 5.6)) &&
          neighbors.some(
            (b) =>
              b.agent !== a &&
              edgeById(this.world, b.agent.edge)!.way === nextEdge.way &&
              Math.cos(b.heading - a.heading) < 0,
          )
        )
        ...
        if (
          ...
          dist < Math.min(65, Math.max(22, (a.speed * a.speed) / 7 + 12))
        ) {
          const r = this.reservations.get(approaching.to);
          if (r && r.id !== a.id && r.until > time)
            stop = Math.min(stop, dist - 5);
          else if (
            distance2(
              player,
            ...
            stop = Math.min(stop, dist - 5);
            blockedByPlayer = true;
          } else if (stop >= dist && gap > 12)
            this.reservations.set(approaching.to, {
              id: a.id,
              until: time + 4,
            });
        }
      ...
      const look = samplePath(
        plan.path,
        Math.min(plan.total, (a.travel || 0) + Math.max(12, a.speed * 1.3)),
        true,
      );
      const angle = Math.abs(
        Math.atan2(
          Math.sin(look.heading - a.heading),
          Math.cos(look.heading - a.heading),
        ),
      );
      const curve = angle / Math.max(12, a.speed * 1.3),
        cornerSpeed = Math.sqrt(
          ((a.race ? 5.8 + traits.accuracy * 2.4 : 3.4) *
            (1 - this.wetness * 0.4)) /
            Math.max(0.001, curve),
        );
      const limit = Math.min(
        a.race ? 60 : edge.speed * (0.8 + seeded(a.id) * 0.2),
        cornerSpeed,
      );
      const ramming =
        !!a.race &&
        traits.aggression > 0.86 &&
        gap < 20 &&
        leadSpeed < a.speed &&
        seeded(a.id * 991 + Math.floor(time * 2)) > 0.94;
      let target = Math.max(
        0,
        Math.min(
          limit,
          Math.sqrt(2 * (a.race ? 8 : 4) * Math.max(0, stop - 1)),
          ramming ? Infinity : (gap - (a.race ? 4 : 5)) / (a.race ? 0.65 : 1.4),
        ),
      );
      if (a.race && !raceRunning) target = 0;
      const oldSpeed = a.speed,
        accel = a.race ? (9500 / 1200) * (1 - a.speed / 90) : 2.7;
      a.speed = Math.max(
        0,
        a.speed +
          clamp(
            target - a.speed,
            -(a.race ? 11 : 7) * dt,
            accel * (1 - this.wetness * 0.35) * dt,
          ),
      );
      a.stuck = a.speed < 0.2 ? a.stuck + dt : 0;
      ...
      if (a.race && !a.race.finished) {
        ...
        if (a.travel >= finish) {
          a.race.finished = true;
          a.race.finishTime = raceElapsed;
        }
      }
      this.sample(a, dt);
      ...
      if (a.race && a.stuck > 10 && distance2(a.point, player) > 180) {
        const candidate = samplePath(
          a.plan!.path,
          Math.min(a.plan!.total, (a.travel || 0) + 16),
          true,
        );
        if (
          this.agents.every(
            (b) => b === a || distance2(b.point, candidate.point) > 12,
          )
        ) {
          a.travel = (a.travel || 0) + 16;
          a.speed = 8;
          a.stuck = 0;
          this.sample(a, dt);
        }
      ...
      if (distance2(a.point, player) < (a.visual ? 330 : 300)) {
        ...
        if (!a.dynamic && distance2(a.point, player) < 18) {
          ...
          body.setLinearVelocity(
            new Vector3(
              Math.sin(a.heading) * a.speed,
              0,
              Math.cos(a.heading) * a.speed,
            ),
          );
        ...
        if (a.dynamic) {
          ...
          if (!a.impact && !(blockedByPlayer && !a.race)) {
            ...
            const f = mesh.getDirection(Vector3.Forward()),
              ...
              angle = Math.atan2(
                Math.sin(a.heading - yaw),
                Math.cos(a.heading - yaw),
              );
            body.applyTorque(
              new Vector3(
                0,
                clamp(
                  angle * 6000 - body.getAngularVelocity().y * 2500,
                  -8000,
                  8000,
                ),
                0,
          ...
          if (
            distance2(mesh.position, player) > 40 &&
            error.length() < 2 &&
            !a.impact
          ) {
            a.dynamic = false;
