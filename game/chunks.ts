...
export class ChunkBudget<T> {
  ...
  constructor(
    public limit: number,
    readonly maxWeight = Infinity,
    private weightOf: (value: T) => number = () => 1,
  ) {}
  get weight() {
  ...
  private trim() {
    while (this.entries.size > this.limit || this.totalWeight > this.maxWeight)
  ...
  get(key: string) {
    ...
    if (item !== undefined) this.touch(key, item);
    return item;
  }
  ...
  invalidateChunks(keys: Iterable<string>) {
    const dirty = new Set(keys);
    for (const key of this.entries.keys())
      if (dirty.has(key.slice(0, key.lastIndexOf('/')))) this.delete(key);
  }
}
...
export class ChunkInstallQueue<T> {
  ...
  drain(install: (value: T) => void, now = () => performance.now()) {
    ...
    while (
      this.entries.size &&
      (installed === 0 || current - started < this.budgetMs)
    ) {
...
export function desiredChunks(
  ...
  const mobile = quality === 'mobile',
    detail = mobile ? 250 : quality === 'high' ? 300 : 500;
  const far = mobile
    ? 400
    : quality === 'high'
      ? 500
  ...
  const lookahead = streaming
    ? Math.min(4 * CHUNK_SIZE, Math.max(0, speedMetersPerSecond) * 18)
    : 0;
  ...
  for (
    let x = streaming ? cx - reach : -10;
    x < (streaming ? cx + reach + 1 : 10);
    x++
  )
    for (
      let z = streaming ? cz - reach : -10;
      z < (streaming ? cz + reach + 1 : 10);
      z++
    ) {
      const center = {
          x: (x + 0.5) * CHUNK_SIZE,
        ...
        d = distance2(center, p);
      const forward =
        ((center.x - p.x) * Math.sin(heading) +
          (center.z - p.z) * Math.cos(heading)) /
        (d || 1);
      const forwardMeters =
        (center.x - p.x) * Math.sin(heading) +
        (center.z - p.z) * Math.cos(heading);
      const lateralMeters = Math.abs(
        (center.x - p.x) * Math.cos(heading) -
          (center.z - p.z) * Math.sin(heading),
      );
      const ahead =
        lookahead > 0 &&
        forwardMeters > 0 &&
...
export function startupDrivingChunks(
  ...
  const cx = Math.floor(p.x / CHUNK_SIZE),
    cz = Math.floor(p.z / CHUNK_SIZE);
  return desiredChunks(p, heading, quality, true)
    .filter((chunk) => {
...
export function criticalChunks(
  p: Point,
  heading: number,
  streaming = false,
  aheadMeters = 70,
  ...
  for (const ahead of [0, aheadMeters])
    for (const dx of [-20, 20])
      for (const dz of [-20, 20]) {
        const x = Math.floor(
            (p.x + Math.sin(heading) * ahead + dx) / CHUNK_SIZE,
          ),
          z = Math.floor((p.z + Math.cos(heading) * ahead + dz) / CHUNK_SIZE);
        if (streaming || (x >= -10 && x < 10 && z >= -10 && z < 10))
          keys.add(`${x},${z}`);
      }
  return [...keys];
}
...
function box(
  ...
  const a = { x: p.x - w / 2, y: p.y, z: p.z - depth / 2 },
    b = { ...a, x: p.x + w / 2 },
    c = { ...b, z: p.z + depth / 2 },
    d = { ...a, z: c.z };
  const up = (v: Point) => ({ ...v, y: v.y + h });
  quad(mesh, a, b, up(b), up(a), color);
  quad(mesh, b, c, up(c), up(b), color);
  quad(mesh, c, d, up(d), up(c), color);
  quad(mesh, d, a, up(a), up(d), color);
  quad(mesh, up(a), up(b), up(c), up(d), color);
}
function ribbon(
  mesh: MeshData,
  a: Point,
...
function roadSurface(
  ...
  const offset = (p: Point, normal: Segment['na'], d: number) => ({
    ...p,
    x: p.x + (normal?.x ?? (b.z - a.z) / length) * d,
    z: p.z + (normal?.z ?? -(b.x - a.x) / length) * d,
  });
  if (s.join) {
    const { nearA, nearB, farA, farB, side } = s.join;
    const centerA = mixPoint(nearA, farA, 0.5),
      centerB = mixPoint(nearB, farB, 0.5);
    const outerA = offset(a, s.na, -w * side),
      outerB = offset(b, s.nb, -w * side);
    return side > 0
      ? [outerA, outerB, centerB, centerA]
      : [centerA, centerB, outerB, outerA];
  }
  return [
    offset(a, s.na, -w),
    offset(b, s.nb, -w),
...
function segmentPolygonSpans(
  ...
  for (let i = 1; i < sorted.length; i++) {
    const start = mixPoint(a, b, sorted[i - 1]),
      end = mixPoint(a, b, sorted[i]),
      mid = mixPoint(start, end, 0.5);
    if (
      polygonContains(mid, outer) &&
      !holes.some((h) => polygonContains(mid, h))
    )
...
function sidewalkShape(s: Segment, side: number) {
  const { a, b, edge } = s,
    length = distance2(a, b) || 1,
    normal = { x: (b.z - a.z) / length, z: -(b.x - a.x) / length },
    w = edge.width / 2,
    outer = w + CURB_WIDTH + SIDEWALK_WIDTH;
  const offset = (
    p: Point,
    n: { x: number; z: number },
    d: number,
    up = CURB_HEIGHT,
  ) => ({ x: p.x + n.x * d * side, y: p.y + up, z: p.z + n.z * d * side });
  const aa = (d: number, up = CURB_HEIGHT) => offset(a, s.na || normal, d, up),
    bb = (d: number, up = CURB_HEIGHT) => offset(b, s.nb || normal, d, up);
  ...
  for (const [axis, bound, sign] of [
    ...
    for (let i = 0; i < polygon.length; i++) {
      const a = polygon[i],
        ...
        ain = (a[axis] - bound) * sign >= 0,
        bin = (b[axis] - bound) * sign >= 0;
      if (ain) output.push(a);
      if (ain !== bin)
        output.push(mixPoint(a, b, (bound - a[axis]) / (b[axis] - a[axis])));
    }
    polygon = output;
  }
  return polygon;
...
export function indexWorld(world: World): Index {
  ...
  const links = new Map<number, Set<number>>(),
    normals = new Map<string, { x: number; z: number; count: number }>();
  const endpoints = new Map<
    number,
    { normal: { x: number; z: number }; x: number; z: number; sign: number }[]
  >();
  const normalAt = (edge: Edge, p: Point, x: number, z: number) => {
    const key = `${edge.way}/${p.x.toFixed(3)}/${p.y.toFixed(3)}/${p.z.toFixed(3)}`,
      normal = normals.get(key) || { x: 0, z: 0, count: 0 };
    normal.x += x;
    normal.z += z;
    normal.count++;
  ...
  for (const edge of world.edges) {
    ...
    for (const [a, b] of [
      ...
      const neighbours = links.get(a) || new Set<number>();
      neighbours.add(b);
      links.set(a, neighbours);
    }
    ...
    for (let i = 0; i < edge.points.length - 1; i++) {
      ...
      const segment: Segment = {
        ...
        index: i,
        station,
        na: normalAt(edge, a, nx, nz),
        nb: normalAt(edge, b, nx, nz),
      };
      for (const [id, normal, sign] of [
        [i === 0 ? edge.from : null, segment.na!, 1],
        [i === edge.points.length - 2 ? edge.to : null, segment.nb!, -1],
      ] as const)
        if (id !== null) {
          const list = endpoints.get(id) || [];
          list.push({ normal, x: nx * sign, z: nz * sign, sign });
          endpoints.set(id, list);
        }
      ...
      if (edge.tunnel || edge.tunnelApproach) {
        ...
        const aa = mixPoint(a, b, -0.25 / length),
          bb = mixPoint(a, b, 1 + 0.25 / length);
        const cavity = roadPrism(
          aa,
          bb,
          edge.width + 2 * (SIDEWALK_WIDTH + CURB_WIDTH + 0.2),
          0.4,
          5.7,
        );
      ...
      const key = tileKey((a.x + b.x) / 2, (a.z + b.z) / 2),
        list = index.owned.get(key) || [];
      list.push(segment);
      index.owned.set(key, list);
      const extra = edge.width / 2 + 35;
      for (
        let x = Math.floor((Math.min(a.x, b.x) - extra) / 250);
        x <= Math.floor((Math.max(a.x, b.x) + extra) / 250);
        x++
      )
        for (
          let z = Math.floor((Math.min(a.z, b.z) - extra) / 250);
          z <= Math.floor((Math.max(a.z, b.z) + extra) / 250);
          z++
        ) {
          const k = `${x},${z}`,
            candidates = index.segments.get(k) || [];
          candidates.push(segment);
          index.segments.set(k, candidates);
        }
    }
  }
  ...
  for (const normal of normals.values()) {
    ...
    const square = normal.x * normal.x + normal.z * normal.z;
  ...
  for (const [id, ends] of endpoints)
    if (
      links.get(id)?.size === 2 &&
      ends.length === 2 &&
      ends[0].normal !== ends[1].normal
    ) {
      const [a, b] = ends,
        x = (a.x - b.x) / 2,
        z = (a.z - b.z) / 2,
      ...
      const factor = Math.min(
        1 / (square || 1),
        1.8 / (Math.sqrt(square) || 1),
      );
      a.normal.x = x * factor * a.sign;
      a.normal.z = z * factor * a.sign;
      b.normal.x = -x * factor * b.sign;
      b.normal.z = -z * factor * b.sign;
    }
  for (const segments of index.owned.values())
    for (const segment of segments) {
      ...
      segment.join = carriagewayJoin(
        segment,
        index.spatial.query(
          boundsOf([segment.a, segment.b], segment.edge.width / 2 + 22),
        ),
        world.drivingSide,
      );
      if (segment.join)
        segment.edge.combinedLanes =
          segment.edge.lanes + segment.join.other.lanes;
    }
  for (const segments of index.owned.values())
    for (const segment of segments)
      for (const side of [-1, 1]) {
        ...
        const { points } = sidewalkShape(segment, side),
          ...
          dx = b.x - a.x,
          dz = b.z - a.z,
          slope = (b.y - a.y) / (dx * dx + dz * dz || 1);
        const height = {
          x: -dx * slope,
          y: 1,
          z: -dz * slope,
          w: -a.y - CURB_HEIGHT + (a.x * dx + a.z * dz) * slope,
        };
        const endpoints = [`${a.x},${a.z}`, `${b.x},${b.z}`].sort().join('/');
        const paving = {
          id: `${segment.edge.way}/${endpoints}/${side}`,
          segment,
          side,
          mask: footprintPrism(points, height, 0.6, 0.6),
        };
  ...
  for (const b of world.buildings) {
    const center = b.footprint.reduce(
      (a, p) => ({
        ...
        y: 0,
        z: a.z + p.z / b.footprint.length,
      }),
      { x: 0, y: 0, z: 0 },
    );
    const key = tileKey(center.x, center.z),
      list = index.buildings.get(key) || [];
    list.push(b);
    index.buildings.set(key, list);
  }
  worldIndices.set(world, index);
  return index;
}
...
export function buildChunk(
  ...
  const index = indexWorld(world),
    segments = index.segments.get(key) || [],
    owned = index.owned.get(key) || [];
  ...
  const result: ChunkData = {
    ...
    paved: { ...empty(), uvs: [] },
    landmarks: empty(),
    facades: [empty(), empty(), empty(), empty()],
    bareFacades: [empty(), empty(), empty(), empty()],
    markings: empty(),
    structures: empty(),
    treeTrunks: empty(),
    buildings: empty(),
  ...
  function ground(x: number, z: number) {
    const cacheKey = `${x},${z}`,
      cached = index.ground.get(cacheKey);
    ...
    for (const s of groundSegments) {
      const projected = projectOnSegment({ x, y: 0, z }, s.a, s.b),
        limit = s.edge.width / 2 + 7;
    ...
    for (const area of index.waters.query(query)) {
      const inside =
        polygonContains(p, area.points) &&
        !(area.holes || []).some((h) => polygonContains(p, h));
      const bank = [area.points, ...(area.holes || [])].some((ring) =>
        ring.some(
          (a, i) =>
  ...
  for (let j = 0; j <= n; j++)
    for (let i = 0; i <= n; i++) {
      const x = x0 + (i * 250) / n,
        z = z0 + (j * 250) / n,
        y = ground(x, z);
      terrain.positions.push(x, y, z);
      const shade = 0.9 + seeded(Math.round(x * 7 + z * 13)) * 0.16;
      terrain.colors!.push(0.105 * shade, 0.16 * shade, 0.125 * shade, 1);
    }
  for (let j = 0; j < n; j++)
    for (let i = 0; i < n; i++) {
      ...
      const k = j * (n + 1) + i;
      terrain.indices.push(k, k + n + 1, k + 1, k + 1, k + n + 1, k + n + 2);
    }
  ...
  const terrainHeight = (x: number, z: number) => {
    ...
    return tx + tz <= 1
      ? h(ix, iz) * (1 - tx - tz) + h(ix + 1, iz) * tx + h(ix, iz + 1) * tz
      : h(ix + 1, iz + 1) * (tx + tz - 1) +
          h(ix, iz + 1) * (1 - tx) +
          h(ix + 1, iz) * (1 - tz);
  };
  for (const s of owned) {
    ...
    const outer =
      w +
      (edge.sidewalkLeft !== false || edge.sidewalkRight !== false
        ? CURB_WIDTH + SIDEWALK_WIDTH
        : 0.2);
    for (const side of [-1, 1]) {
      if (s.join?.side === side) continue;
      if (!sidewalkOn(edge, side)) continue;
      const { aa, bb, points } = sidewalkShape(s, side),
        bounds = boundsOf(points);
      const candidates = index.paving.query(bounds),
        own = candidates.find((p) => p.segment === s && p.side === side)!;
      const roads = index.spatial
        .query(bounds)
      ...
      const walls = [
        ...roads,
        ...candidates.filter((p) => p !== own).map((p) => p.mask),
      ];
      for (const [left, right, color] of [
        [w, w + CURB_WIDTH, [0.58, 0.59, 0.57]],
        [w + CURB_WIDTH, outer, [0.4, 0.42, 0.42]],
      ] as [number, number, Colour][])
        sidewalkPolygon(
          result.sidewalks!,
          [aa(left), bb(left), bb(right), aa(right)],
          color,
          tops,
        );
      sidewalkPolygon(
        result.sidewalks!,
        [aa(w, 0), bb(w, 0), bb(w), aa(w)],
        [0.52, 0.53, 0.51],
        walls,
      );
      const groundA = aa(outer, 0),
        groundB = bb(outer, 0);
      if (edge.bridge || edge.tunnel || edge.tunnelApproach) {
        groundA.y = aa(outer, -0.08).y;
        groundB.y = bb(outer, -0.08).y;
      } else {
        groundA.y = terrainHeight(groundA.x, groundA.z) - 0.02;
        groundB.y = terrainHeight(groundB.x, groundB.z) - 0.02;
      }
      sidewalkPolygon(
        result.sidewalks!,
        [groundA, aa(outer), bb(outer), groundB],
        [0.34, 0.35, 0.35],
        walls,
      );
    }
    if (!edge.bridge && !edge.tunnel)
      for (const side of [-1, 1]) {
        if (s.join?.side === side) continue;
        const len = distance2(a, b) || 1,
          normal = { x: (b.z - a.z) / len, z: -(b.x - a.x) / len };
        const offset = (p: Point, n: { x: number; z: number }, d: number) => ({
          x: p.x + n.x * d * side,
          y: p.y - 0.08,
          z: p.z + n.z * d * side,
        });
        const sideOuter =
            w + (sidewalkOn(edge, side) ? CURB_WIDTH + SIDEWALK_WIDTH : 0.2),
          aa = offset(a, s.na || normal, sideOuter),
          bb = offset(b, s.nb || normal, sideOuter);
        const cc = offset(b, s.nb || normal, w + 7),
          dd = offset(a, s.na || normal, w + 7);
        cc.y = terrainHeight(cc.x, cc.z) + 0.02;
        dd.y = terrainHeight(dd.x, dd.z) + 0.02;
        if (side > 0)
          quad(result.shoulders, aa, bb, cc, dd, [0.23, 0.27, 0.22]);
        else quad(result.shoulders, dd, cc, bb, aa, [0.23, 0.27, 0.22]);
      }
    ribbon(
      result.road,
      a,
    ...
    const junctionPoints = [
      ...
      s.index === edge.points.length - 2 && index.junctions.has(edge.to)
        ? b
        : null,
    ...
    for (const p of junctionPoints) {
      ...
      circles.add(id);
      for (let k = 0; k < 12; k++) {
        const base = result.road.positions.length / 3,
          aa = (k / 12) * Math.PI * 2,
          bb = ((k + 1) / 12) * Math.PI * 2;
        result.road.positions.push(
          p.x,
          p.y - 0.015,
          p.z,
          p.x + Math.cos(aa) * w,
    ...
    const crossing = segments.some(
      (other) =>
        ...
        other.edge.layer === edge.layer &&
        Math.abs(other.a.y - a.y) < 2 &&
        projectOnSegment(mixPoint(a, b, 0.5), other.a, other.b).distance <
          other.edge.width / 2 + 1,
    );
    if (lod === 0 && !crossing) {
      if (s.join?.side !== -1)
        ribbon(
      ...
      const separators =
        edge.laneProfile?.separators ||
        Array.from({ length: Math.max(0, edge.lanes - 1) }, (_, i) => ({
          offset: -w + ((i + 1) * edge.width) / edge.lanes,
          kind: 'lane' as const,
        }));
      const length = distance2(a, b);
      for (const line of separators)
        for (const [start, end] of dashSpans(
          s.station,
          length,
          edge.laneProfile?.direction || 1,
        )) {
          const normal = (t: number) =>
            s.na && s.nb
              ? {
                  x: s.na.x + (s.nb.x - s.na.x) * t,
                  z: s.na.z + (s.nb.z - s.na.z) * t,
                }
              : undefined;
          const half = line.kind === 'divider' ? 0.075 : 0.055;
          ribbon(
            result.markings,
            mixPoint(a, b, start / length),
            mixPoint(a, b, end / length),
            line.offset - half,
            line.offset + half,
            0.05,
            [0.82, 0.84, 0.8],
    ...
    if (edge.bridge) {
      ...
      for (const side of [-1, 1]) {
        ...
        const aa = {
            ...
            y: a.y,
            z: a.z + (s.na?.z ?? nz) * outer * side,
          },
          bb = {
            x: b.x + (s.nb?.x ?? nx) * outer * side,
            y: b.y,
      ...
      for (const d of periodicOffsets(
        ...
        const p = mixPoint(a, b, d / distance2(a, b)),
          base = sampleElevation(world.elevation, p.x, p.z);
        const free = segments.every(
          (other) =>
            other.edge.layer >= edge.layer ||
            projectOnSegment(p, other.a, other.b).distance >
              other.edge.width / 2 + SIDEWALK_WIDTH + 1,
        );
        if (free && p.y - base > 2)
          box(
            result.structures,
            { ...p, y: base },
            1.4,
            p.y - base - BRIDGE_DECK_THICKNESS,
            1.4,
            [0.23, 0.27, 0.28],
          );
      }
    ...
    if (edge.tunnel) {
      const length = distance2(a, b),
        nx = (b.z - a.z) / length,
        nz = -(b.x - a.x) / length;
      const offset = (p: Point, side: number) => ({
        x: p.x + nx * (outer + 0.2) * side,
        y: p.y - 0.3,
        z: p.z + nz * (outer + 0.2) * side,
      });
      for (const side of [-1, 1]) {
        const aa = offset(a, side),
          bb = offset(b, side);
        quad(
          result.structures,
          aa,
    ...
    if (
      edge.blocked &&
      (!edge.blockedReasons?.length ||
        edge.blockedReasons.some((reason) => reason !== 'coverage')) &&
      !edge.unloaded &&
      s.index === 0 &&
      (world.loadedTiles || (Math.abs(a.x) < 2490 && Math.abs(a.z) < 2490))
    ) {
      const length = distance2(a, b),
        nx = (b.z - a.z) / length,
        nz = -(b.x - a.x) / length;
      for (let k = -Math.floor(w / 1.4); k <= Math.floor(w / 1.4); k++)
        box(
          result.structures,
          { x: a.x + nx * k * 1.4, y: a.y, z: a.z + nz * k * 1.4 },
          1.3,
          0.9,
          1.3,
          k % 2 ? [0.8, 0.33, 0.12] : [0.67, 0.68, 0.58],
        );
    }
    if (lod === 0 && !edge.tunnel && s.join?.side !== 1)
      for (const d of periodicOffsets(
        s.station,
        distance2(a, b),
        edge.laneProfile?.direction || 1,
      )) {
        const l = distance2(a, b),
          anchor = mixPoint(a, b, d / l),
          p = {
            x: anchor.x + ((b.z - a.z) / l) * (w + 1.6),
            y: anchor.y,
            z: anchor.z - ((b.x - a.x) / l) * (w + 1.6),
          };
        const free = segments.every(
          (other) =>
            Math.abs(other.a.y - p.y) > 3 ||
            projectOnSegment(p, other.a, other.b).distance >
              other.edge.width / 2 + 0.6,
        );
        if (free) {
          result.breakables.push({ kind: 'pole', point: p, heading: 0 });
          result.lamps.push({ ...p, y: p.y + 6.7 });
        }
      }
  ...
  const models = worldLandmarks(world).filter(
    (model) => model.key === key && (lod === 0 || model.asset.far),
  );
  ...
  for (const building of index.buildings.get(key) || []) {
    if (
      models.some(
        (model) =>
          model.asset.kind === 'building' &&
          model.asset.osm.some(
            (ref) =>
              ref.type === (building.osmType || 'way') &&
              ref.id === building.id,
          ),
      )
    )
    ...
    if (closeCourtyards && !significant) {
      const publicSegments = index.spatial
        .query(boundsOf(building.footprint, 24))
        .filter((s) => s.edge.category !== 'service');
      for (let i = 0; i < building.footprint.length; i++) {
        const a = building.footprint[i],
          b = building.footprint[(i + 1) % building.footprint.length],
          length = distance2(a, b) || 1;
        if (
          publicSegments.some((s) => {
            ...
            if (parallel < 0.5) return false;
            return [a, mixPoint(a, b, 0.5), b].some(
              (p) => projectOnSegment(p, s.a, s.b).distance <= 24,
            );
          })
        )
          frontageEdges.add(i);
      ...
      if (!frontageEdges.size) {
        const grounded = {
          ...
          holes: building.holes?.map((r) =>
            r.map((p) => ({ ...p, y: terrainHeight(p.x, p.z) })),
          ),
        };
    ...
    const groundRing = (ring: Point[]) =>
      ring.flatMap((p, i) => {
        ...
        for (const [a, b] of [
          ...
          if (a !== b)
            for (
              let line = Math.ceil(Math.min(a, b) / 12.5);
              line <= Math.floor(Math.max(a, b) / 12.5);
              line++
            )
              cuts.add((line * 12.5 - a) / (b - a));
        return [...cuts].map((t) => {
          const v = mixPoint(p, q, t);
          return { ...v, y: terrainHeight(v.x, v.z) };
        });
      });
    ...
    const grounded = {
      ...
      holes: building.holes?.map((r) => r.map(groundVertex)),
    };
    const roads = index.spatial
      .query(boundsOf(building.footprint))
      .filter((s) => !s.edge.tunnel && !s.edge.bridge)
      .map((s) =>
        roadPrism(
          s.a,
          s.b,
    ...
    for (const s of index.spatial
      .query(boundsOf(building.footprint))
      .filter((s) => s.edge.passage)) {
      const length = distance2(s.a, s.b) || 1,
        nx = (s.b.z - s.a.z) / length,
        nz = -(s.b.x - s.a.x) / length;
      for (const side of [-1, 1]) {
        const d = (s.edge.width + 0.6) / 2,
          aa = { ...s.a, x: s.a.x + nx * d * side, z: s.a.z + nz * d * side },
          bb = { ...s.b, x: s.b.x + nx * d * side, z: s.b.z + nz * d * side };
        for (const [p, q] of segmentPolygonSpans(
          aa,
          bb,
          grounded.footprint,
          grounded.holes,
  ...
  for (const area of world.areas) {
    ...
    if (area.kind === 'water') {
      ...
      for (let i = 0; i < 24; i++) {
        const p = {
          x: x0 + seeded(area.id + cx * 771 + cz * 991 + i * 31) * 250,
          y: 0,
          z: z0 + seeded(area.id + cx * 887 + cz * 773 + i * 87) * 250,
        };
        if (
          !polygonContains(p, area.points) ||
          (area.holes || []).some((h) => polygonContains(p, h)) ||
          segments.some(
            (s) =>
              projectOnSegment(p, s.a, s.b).distance < s.edge.width / 2 + 3,
          )
        )
          continue;
        p.y = ground(p.x, p.z);
  ...
  if (lod === 0)
    for (const tree of world.trees)
      if (
        tileKey(tree.x, tree.z) === key &&
        segments.every(
          (s) =>
            projectOnSegment(tree, s.a, s.b).distance > s.edge.width / 2 + 2,
        )
      )
  ...
  for (const s of segments)
    if (!s.edge.bridge && !s.edge.tunnel) {
      ...
      const cut = roadPrism(aa, bb, s.edge.width + 14, 10000, 10000);
      roadCuts.add(cut, cut.bounds);
      const polygon = roadSurface(s);
      for (const triangle of [
        [polygon[0], polygon[1], polygon[2]],
        [polygon[0], polygon[2], polygon[3]],
      ] as [Point, Point, Point][]) {
        const mask = surfacePrism(triangle, 0.1, 10000);
        if (mask) surfaceCuts.add(mask, mask.bounds);
      }
      for (const p of [
        s.index === 0 && index.junctions.has(s.edge.from) ? s.a : null,
        s.index === s.edge.points.length - 2 && index.junctions.has(s.edge.to)
          ? s.b
          : null,
      ])
        if (p) {
          const ring = Array.from({ length: 12 }, (_, i) => ({
            ...p,
            x: p.x + (Math.cos((i / 12) * Math.PI * 2) * s.edge.width) / 2,
            z: p.z + (Math.sin((i / 12) * Math.PI * 2) * s.edge.width) / 2,
          }));
